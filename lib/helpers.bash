...
function _load_bashstrap_files() {
  subdirectory="$1"
  if [ ! -d "${BASHSTRAP}/${subdirectory}" ]
  then
    continue
  fi
  FILES="${BASHSTRAP}/${subdirectory}/*.bash"
...
_bash-it-aliases ()
{
    _about 'summarizes available bashstrap aliases'
    _group 'lib'
...
{
    _about 'summarizes available bashstrap completions'
    _group 'lib'
...
_bash-it-plugins ()
{
    _about 'summarizes available bashstrap plugins'
    _group 'lib'
...
_bash-it-describe ()
{
    _about 'summarizes available bashstrap components'
    ...
    for f in $BASHSTRAP/$subdirectory/available/*.bash
    do
        if [ -e $BASHSTRAP/$subdirectory/enabled/$(basename $f) ]; then
            enabled='x'
        else
            enabled=' '
        fi
...
_disable-plugin ()
{
    _about 'disables bashstrap plugin'
...
_disable-alias ()
{
    _about 'disables bashstrap alias'
    _param '1: alias name'
...
_disable-completion ()
{
    _about 'disables bashstrap completion'
...
_disable-thing ()
{
    _about 'disables a bashstrap component'
    _param '1: subdirectory'
    _param '2: file_type'
    ...
    if [ "$file_entity" = "all" ]; then
        ...
        do
            plugin=$(basename $f)
            if [ -e $BASHSTRAP/$subdirectory/enabled/$plugin ]; then
                rm $BASHSTRAP/$subdirectory/enabled/$(basename $plugin)
            fi
        done
    else
        typeset plugin=$(command ls $BASHSTRAP/$subdirectory/enabled/$file_entity.*bash 2>/dev/null | head -1)
        if [ -z "$plugin" ]; then
...
_enable-plugin ()
{
    _about 'enables bashstrap plugin'
    _param '1: plugin name'
...
_enable-alias ()
{
    _about 'enables bashstrap alias'
    _param '1: alias name'
...
_enable-completion ()
{
    _about 'enables bashstrap completion'
    _param '1: completion name'
...
_enable-thing ()
{
    ...
    _about 'enables a bashstrap component'
    _param '1: subdirectory'
    _param '2: file_type'
    _param '3: file_entity'
    ...
    if [ "$file_entity" = "all" ]; then
        ...
        do
            plugin=$(basename $f)
            if [ ! -h $BASHSTRAP/$subdirectory/enabled/$plugin ]; then
                ln -s ../available/$plugin $BASHSTRAP/$subdirectory/enabled/$plugin
            fi
        done
    else
        typeset plugin=$(command ls $BASHSTRAP/$subdirectory/available/$file_entity.*bash 2>/dev/null | head -1)
        if [ -z "$plugin" ]; then
        ...
        mkdir -p $BASHSTRAP/$subdirectory/enabled
...
_help-aliases()
{
    ...
    if [ -n "$1" ]; then
        cat $BASHSTRAP/aliases/available/$1.aliases.bash | metafor alias | sed "s/$/'/"
    else
        typeset f
        for f in $BASHSTRAP/aliases/enabled/*
        do
